...
if Config.UseVoiceHud then
	...
	function drawVoiceCircle(radius, duration)
		local playerPed = PlayerPedId()
		local playerCoords = GetEntityCoords(playerPed)

		-- Dessiner le cercle (utiliser un marqueur de type 1 pour un cercle)
		local endTime = GetGameTimer() + (duration * 1000) -- Convertir la durée en millisecondes

		while GetGameTimer() < endTime do
			Citizen.Wait(0) -- Attendre pour éviter de bloquer le fil principal

			-- Faire pulser le cercle
			local currentRadius = radius + math.sin(GetGameTimer() / 100) * 0.2

			-- Dessiner le marqueur (cercle bleu)
			DrawMarker(1, playerCoords.x, playerCoords.y, playerCoords.z - 0.5, 0, 0, 0, 0, 0, 0, currentRadius, currentRadius, 0.1, 0, 255, 0, 150, false, true, 2, false, nil, nil, false, false)

			-- Dessiner un contour blanc autour du cercle
			DrawMarker(1, playerCoords.x, playerCoords.y, playerCoords.z - 0.5, 0, 0, 0, 0, 0, 0, currentRadius + 0.1, currentRadius + 0.1, 0.1, 255, 255, 255, 100, false, true, 2, false, nil, nil, false, false)
		end
	end

	-- Événement pour changer de mode de voix
	RegisterNetEvent('pma-voice:setTalkingMode')
	AddEventHandler('pma-voice:setTalkingMode', function(voiceMode)
		local voiceModes = {
			[1] = "Chuchoter",  -- Mode de voix 1
			[2] = "Normal",     -- Mode de voix 2
			[3] = "Crier"       -- Mode de voix 3
		}

		local voiceModeName = voiceModes[voiceMode] or "Inconnu"

		lib.notify({
			title = 'Modification de la voix',
			description = "Mode de voix: " .. voiceModeName,
			position = 'top-center',
			icon = 'microphone',
    		iconColor = '#00ff00',
			iconAnimation = 'shake'
		...
		-- Déterminer le rayon en fonction du mode de voix
		local radius = 0
		if voiceMode == 1 then
			radius = 3.0 -- Whisper
		...
		elseif voiceMode == 3 then
			radius = 15.0 -- Shouting
		end

		drawVoiceCircle(radius, 0.5) -- Appeler la fonction pour dessiner le cercle pendant 2 secondes
	end)
